...
import type { PRD, PRDInsert, PRDStatus, Json } from '@/types/database';
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (status && status.length > 0) {
      query = query.in('status', status as PRDStatus[]);
    }
    if (project_id) {
    ...
    if (error) {
      ...
      return errorResponse('Failed to fetch PRDs', 500, headers);
    }

    const response: PaginatedResponse<PRD> = {
      data: data || [],
      pagination: {
        total: count || 0,
        limit,
    ...
    console.error('Unexpected error in GET /api/v1/prds:', err);
...
export async function POST(request: NextRequest) {
  ...
  try {
    const body = await request.json() as Partial<PRDInsert>;
    ...
    const { PRD_SECTIONS: defaultSections } = await import('@/types/database');

    // Convert sections to JSON-compatible format
    const sectionsJson: Json = body.sections 
      ? JSON.parse(JSON.stringify(body.sections))
      : JSON.parse(JSON.stringify(defaultSections));

    const newPRD: PRDInsert = {
      title: body.title,
    ...
    console.error('Unexpected error in POST /api/v1/prds:', err);
    return errorResponse('Internal server error', 500, headers);
  }
}
